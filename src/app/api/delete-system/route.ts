...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (id && isUUID(id)) {
      ...
      const { error } = await supabaseService.from("systems").delete().eq("id", id);
    ...
    if (name) {
      const { data: rows, error: queryErr } = await supabaseService.from("systems").select("id").eq("name", name);
